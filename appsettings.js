// appsettings.js - Trip Tracker App Settings
const AppSettingsManager = {
    appSettings: {},
    
    init: function() {
        this.loadAppSettings();
        this.renderAppSettingsPage();
        this.bindEvents();
    },
...
<ins class="adsbygoogle"
    ...
    renderAppSettingsPage: function() {
        const appSettingsDiv = document.getElementById('app-settings');
        if (!appSettingsDiv) return;
    ...
    renderTravelPreferencesSection: function() {
        return `
    ...
    renderCurrencyOptions: function() {
        const currencies = (typeof AppConfig !== 'undefined') ? AppConfig.config.currencies : ['USD', 'EUR', 'GBP', 'INR'];
        const currencyNames = (typeof AppConfig !== 'undefined') ? AppConfig.config.currencyNames : {};
        
        return currencies.map(currency => {
            const name = currencyNames[currency] || currency;
            const selected = this.appSettings.preferences.homeCurrency === currency ? 'selected' : '';
            return `<option value="${currency}" ${selected}>${currency} - ${name}</option>`;
        }).join('');
    },
    
    renderCategoryOptions: function() {
        const categories = (typeof AppConfig !== 'undefined') ? AppConfig.config.categories : ['Food', 'Transport', 'Accommodation', 'Shopping', 'Other'];
        
        return categories.map(category => {
            const selected = this.appSettings.preferences.defaultExpenseCategory === category ? 'selected' : '';
            return `<option value="${category}" ${selected}>${category}</option>`;
        }).join('');
    },
    
    renderPaymentModeOptions: function() {
        const modes = (typeof AppConfig !== 'undefined') ? AppConfig.config.modes : ['Cash', 'Credit Card', 'Prepaid Card'];
        
        return modes.map(mode => {
            const selected = this.appSettings.preferences.defaultPaymentMode === mode ? 'selected' : '';
    ...
    bindEvents: function() {
        document.addEventListener('change', (e) => {
            switch(e.target.id) {
                case 'home-currency':
                    this.appSettings.preferences.homeCurrency = e.target.value;
                    this.saveAppSettings();
                    break;
                case 'home-location':
                    this.appSettings.preferences.homeLocation = e.target.value;
                    this.saveAppSettings();
                    break;
                case 'default-category':
                    this.appSettings.preferences.defaultExpenseCategory = e.target.value;
                    this.saveAppSettings();
                    break;
                case 'default-payment-mode':
                    this.appSettings.preferences.defaultPaymentMode = e.target.value;
                    this.saveAppSettings();
                    break;
            }
        });
        ...
        document.addEventListener('click', (e) => {
            if (e.target.id === 'reset-trip-settings-btn') {
    ...
    handleResetSettings: function() {
        const confirmationMessage = 'Are you sure you want to reset all trip settings to their default values?';
    ...
    saveAppSettings: function() {
        try {
            const storageKey = this.getStorageKey();
            localStorage.setItem(storageKey, JSON.stringify(this.appSettings));
        } catch (error) {
    ...
    getStorageKey: function() {
        if (typeof AppConfig !== 'undefined') {
            return AppConfig.config.storageKeys.appSettings;
        }
        return 'lipikit_trip_settings';
    },
    
    // Public API methods for Trip Tracker
    getHomeCurrency: function() {
        return this.appSettings.preferences.homeCurrency || 'USD';
    },
    
    getHomeLocation: function() {
        return this.appSettings.preferences.homeLocation || '';
    },
    
    getDefaultCategory: function() {
        return this.appSettings.preferences.defaultExpenseCategory || 'Food';
    },
    
    getDefaultPaymentMode: function() {
        return this.appSettings.preferences.defaultPaymentMode || 'Cash';
    },
    
    // Helper method to get trip creation defaults
    getTripDefaults: function() {
        return {
            baseCurrency: this.getHomeCurrency(),
            location: this.getHomeLocation(),
            defaultCategory: this.getDefaultCategory(),
            defaultPaymentMode: this.getDefaultPaymentMode()
        };
    },
    
    // Stub for backwards compatibility
    applyAllSettings: function() {
        console.debug("applyAllSettings() is deprecated and no longer needed.");
    }
};
// Ensure global AppSettings is available for DatasetManager
