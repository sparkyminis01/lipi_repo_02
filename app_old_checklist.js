...
getUserTripDatabase: async function(tripId) {
    ...
    showMessage: function(title, message, isConfirmation = false) {
    ...
    return `
        <div class="adsense-container" data-ad-initialized="false" style="margin: 1rem 0;">
            <div style="min-height:90px; background:#f9f9f9; border:2px dashed #ddd; 
                ...
                [No AdSense Configuration Found]
            </div>
        </div>
    `;
},
    ...
    adContainers.forEach((container, index) => {
        ...
        try {
            ...
            if (typeof window.adsbygoogle !== 'undefined' && window.adsbygoogle) {
                ...
                console.warn('AdSense script not available, showing placeholder');
            ...
            ${Object.entries(categorizedItems).map(([category, items]) => `
                <div class="checklist-category">
                    ...
                    <div class="checklist-table">
                        ${items.map(item => `
                            <div class="checklist-row" data-id="${item.checklistId}">
                                <div class="checklist-checkbox">
                                    <input type="checkbox" 
                                           id="check-${item.checklistId}"
                                           ${item.status === 'Done' ? 'checked' : ''} 
                                           data-checklist-id="${item.checklistId}">
                                </div>
                                <div class="item-label">
                                    <label for="check-${item.checklistId}">${item.text}</label>
                                </div>
                                <div class="item-help">
                                    <button class="help-btn" 
                                            data-tags="${this.escapeHtml(item.tags || '')}"
...
categorizeMasterChecklist(items, existingIds = null) {
    ...
    const processedItems = existingIds ? masterItems : items.map(item => {
        const masterItem = masterItems.find(mi => mi.id === item.checklistId);
        return { ...masterItem, ...item };
    });
...
if (tabBtn && checklistViewRoot.contains(tabBtn)) {
    ...
    return;
            ...
            // Add/Remove toggle in Customize tab - use closest so clicks on inner text still work
            ...
            if (helpBtn && checklistViewRoot.contains(helpBtn)) {
                const tags = helpBtn.dataset.tags;
                const description = helpBtn.dataset.description;
                this.showChecklistHelp(tags, description);
                return;
            }
...
if (backBtn && checklistViewRoot.contains(backBtn)) {
    ...
    showChecklistHelp(tags, description) {
        let content = '';
        if (tags) {
            content += `<p class="help-tags">${tags}</p>`;
        }
        if (description) {
            // Convert URLs to clickable links
            const urlRegex = /(https?:\/\/[^\s]+)/g;
            const descWithLinks = description.replace(urlRegex, '<a href="$1" target="_blank">$1</a>');
            content += `<p class="help-description">${descWithLinks}</p>`;
        }

        this.showMessage('Item Help', content || 'No additional information available.');
    }
};