const ProfileManager = {
    ...
    setupAuthStateListener: function() {
        ...
        this.authStateListener = this.firebaseAuth.onAuthStateChanged(async (firebaseUser) => {
            if (firebaseUser) {
                ...
                await this.checkAndUpdateProStatus();
            } else {
    ...
    async handleAuthenticatedUser(firebaseUser) {
        ...
        // Load user profile from Firestore to get actual plan/credits
        try {
            const userDoc = await this.firestore.collection('users').doc(firebaseUser.uid).get();
            if (userDoc.exists) {
                const userData = userDoc.data();
                this.currentUser.plan = userData.plan || 'free';
                this.currentUser.tripCreditsUsed = userData.tripCreditsUsed || 0;
                this.currentUser.proExpiresAt = userData.proExpiresAt || null;
            }
        } catch (error) {
        ...
        userDropdown.innerHTML = `
            ...
            <div class="dropdown-menu hidden">
                <div class="user-info" style="padding: 0.75rem; border-bottom: 1px solid #f3f4f6;">
                    <div style="font-weight: 600; color: #374151; display: flex; align-items: center; justify-content: space-between; gap: 0.5rem;">
                        ${this.currentUser.displayName}
                        ${this.currentUser.plan === 'pro' 
                            ? '<span style="background: linear-gradient(135deg, #10b981, #059669); color: white; padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.7rem; font-weight: 700;">✓ PRO</span>'
                            : '<span style="background: #94a3b8; color: white; padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.7rem; font-weight: 700;">FREE</span>'}
                    </div>
                    <div style="font-size: 0.8rem; color: #6b7280;">${this.currentUser.email}</div>
                    ${this.currentUser.plan !== 'pro' 
                        ? `<div style="margin-top: 0.5rem; font-size: 0.75rem; color: var(--primary-color); font-weight: 600;">
                             Credits: ${Math.max(0, 3 - (this.currentUser.tripCreditsUsed || 0))}/3
                           </div>`
    ...
    async checkAndUpdateProStatus() {
        const user = this.currentUser;
        if (!user || user.plan !== 'pro' || !user.proExpiresAt) return;
        ...
        if (now > expiresAt) {
            try {
                ...
                this.currentUser.proExpiresAt = null;
                this.currentUser.tripCreditsUsed = 0;

                console.log('Pro plan expired, downgraded to free');
                
                this.showError('Your pro plan has expired. You\'re back on the free plan.');
                
                this.renderProfileSection();
                if (typeof AppManager !== 'undefined' && AppManager.renderApp) {
                    AppManager.renderApp();
                }
            } catch (error) {
